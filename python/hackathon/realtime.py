...
def get_user_input(preprocessor, model):
    print("Enter the following details:")

    # Getting user inputs
    person_age = float(input("Person's age: "))
    person_income = float(input("Person's annual income: "))
    loan_amnt = float(input("Loan amount: "))
    amount_paid = float(input("Amount paid for electricity: "))
    person_home_ownership = input("Person's home ownership status (Own/Rent): ")
    is_urban = int(input("Is the person living in an urban area? (1 for Yes, 0 for No): "))
    num_people = int(input("Number of people in the household: "))
    cb_person_default_on_file = input("Has the person defaulted on credit before? (Yes/No): ")
    ...
    # Display the result
    if prediction == 0:
        print("Prediction: The loan is Low Risk (Will not default).")
    elif prediction == 1:
        print("Prediction: The loan is High Risk (Will default).")
    else:
        print("Prediction: Loan grade classification.")
...
def main():
    ...
    while True:
        processed_data = get_user_input(full_pipeline.named_steps['preprocessor'], full_pipeline.named_steps['classifier'])
        q=input('entering more (y/n)?')
        if q.lower()=='n':
            break
